jQuery(window).load(function(){
    initializePage();
    jQuery("#referencesDialog").dialog({autoOpen: false},{width:jQuery(window).width()*0.75},{height:jQuery(window).height()*0.75});
    jQuery("#referencesButton").click(function(){jQuery("#referencesDialog").dialog("open")});
    });
    
    jQuery(window).resize(function() { 
        jQuery("#referencesDialog").dialog("option", "position", { my: "center", at: "center", of: window });
...
var implementnew = [
 ...
 jQuery("#mainImage").animate({opacity:1},2000,"easeInExpo",function(){ jQuery('body').css("background-color","#BE90D4"); loadPopupstream();});  
 jQuery("#referencesButton").animate({opacity:1},500,"swing");
 jQuery("#BrowserCompatText").animate({opacity:0.5},500,"swing");
 ...
 anno.addHandler('onPopupShown', function() {
     ...
     jQuery('body').animate({scrollTop: document.body.scrollHeight-jQuery('.annotorious-popup-text').offset().top}, 300, anno.showAnnotations());
 });
